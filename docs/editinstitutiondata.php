...
$stmt = $conn->prepare("SELECT title, session, type, filename, content FROM institutional_data WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->bind_result($title, $session, $type, $filename, $content);
$stmt->fetch();
$stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $new_session = trim($_POST["session"]);
    $new_content = null;
    $new_filename = $filename; // default to old file

    if ($type === "text") {
        $new_content = trim($_POST["content"]);
    }

    if (($type === "pdf" || $type === "image") && !empty($_FILES["file"]["name"])) {
        $uploadDir = "institutional_upload/";
        // Optionally delete old file
        if ($filename && file_exists($uploadDir . $filename)) {
            unlink($uploadDir . $filename);
        }

        $new_filename = basename($_FILES["file"]["name"]);
        move_uploaded_file($_FILES["file"]["tmp_name"], $uploadDir . $new_filename);
    }

    $stmt = $conn->prepare("UPDATE institutional_data SET title = ?, session = ?, content = ?, filename = ? WHERE id = ?");
    $stmt->bind_param("ssssi", $new_title, $new_session, $new_content, $new_filename, $id);
    $stmt->execute();
    $stmt->close();

    header("Location: manageinstitutiondata.php?success=updated");
    exit();
}
?>
            ...
            <div class="kingster-page-title-wrap  kingster-style-medium kingster-center-align">
                ...
                <div class="kingster-page-title-container kingster-container">
                    <div class="kingster-page-title-content kingster-item-pdlr">
                        <h1 class="kingster-page-title">EDIT INSTITUTIONAL DATA</h1></div>
                </div>
            </div>
...
<div class="gdlr-core-pbf-element">
    <div class="gdlr-core-blog-item gdlr-core-item-pdb clearfix gdlr-core-style-blog-full-with-frame" style="padding-bottom: 40px;">
        <div class="gdlr-core-blog-item-holder gdlr-core-js-2 clearfix" data-layout="fitrows">
            ...
            <!-- Upcoming Events -->
            <Center> <h3>📝 EDIT INSTITUTIONAL DATA</h3></Center>
    ...
    <input type="text" name="session" value="<?= htmlspecialchars($session) ?>" required><br><br>

    <?php if ($type === 'text'): ?>
        <label>Text Content:</label><br>
        <textarea name="content" rows="5" cols="50"><?= htmlspecialchars($content) ?></textarea><br><br>
    <?php else: ?>
        <label>Current File:</label><br>
        <a href="institutional_upload/<?= htmlspecialchars($filename) ?>" target="_blank"><?= $filename ?></a><br><br>
        <label>Replace File:</label><br>
